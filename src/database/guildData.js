...
const { databaseQuerys } = require("./const");
const { NoichuChannelConfig } = require("../typings/index.js");
/**
 ...
 */
async function registGuild(guildId) {
  try {
    const isDuplicate = async (guiidlId) => {
    ...
    if (await isDuplicate(guildId)) return false;
    else {
      ...
      const query = databaseQuerys.NO_DUPLICATE_INSERT_GUILD;
      const values = [newGuildConfig.id, newGuildConfig.name, newGuildConfig.maxNoichuChannel];
      await runQuery(query, values);
      return true;
    }
  } catch (err) {
    logger.error(`Erron on registing guild with id ${guildId}: ${err}`);
    return null;
  }
}
module.exports = {
  ...
  async getNumberOfNoichuChannelInGuild(guildId) {
    try {
      const query = databaseQuerys.getNumberOf.NOICHU_CHANNEL_IN_GUILD;
      const values = [guildId];
      const result = await runQuery(query, values);
